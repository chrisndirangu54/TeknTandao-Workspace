...
class _GenericEnterpriseModuleScreenState
  ...
  Future<void> _openEditor([Map<String, dynamic>? existing]) async {
    ...
    final controllers = <String, TextEditingController>{
      for (final field in blueprint.fields)
        field.key: TextEditingController(text: existing?[field.key]?.toString() ?? ''),
    };
  ...
  Future<void> _advance(Map<String, dynamic> item) async {
    ...
    final next = blueprint.statuses[(currentIndex < 0
            ? 0
            : (currentIndex + 1).clamp(0, blueprint.statuses.length - 1))];
    await store.call('saveRecord', {
      'appId': module.id,
      'id': item['id'],
      'record': {
        'title': (item['title'] ?? item['name'] ?? blueprint.entityLabel).toString(),
        'status': next,
      },
    });
  }
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ...
            _statCard('Monthly plan', '${kes(module.monthlyPriceKes)}/mo',
                Icons.sell_rounded),
          ],
        ),
      ],
  ...
  Widget _emptyState(bool noRecords) => Card(
        child: Padding(
          ...
          child: Row(
            children: [
              Icon(noRecords ? Icons.inbox_outlined : Icons.search_off_rounded,
                  color: module.color),
              const SizedBox(width: 12),
              Expanded(
                child: Text(noRecords
  ...
  Widget _recordTile(Map<String, dynamic> item) {
    final title = (item['title'] ?? item['name'] ?? blueprint.entityLabel).toString();
    final status = item['status']?.toString() ?? blueprint.statuses.first;
    final detail = blueprint.fields
        .where((field) => item[field.key]?.toString().trim().isNotEmpty == true)
        .take(3)
        .map((field) => '${field.label}: ${item[field.key]}')
        .join(' · ');
    final statusIndex = blueprint.statuses.indexOf(status);
    final canAdvance = statusIndex >= 0 && statusIndex < blueprint.statuses.length - 1;
